---
tags:
  - math
aliases:
  - Normal Distribution
title: normal distribution
description:
parent nodes:
  - "[[202602072325 - distribution|Distribution]]"
annotation-target:
published on:
---

- The Normal Distribution (also known as the <mark style="background: #BBFABBA6;">Gaussian Distribution</mark> or the <mark style="background: #ADCCFFA6;">Bell Curve</mark>) is a continuous probability distribution that is symmetrical around the [[202602062110 - mean|mean]] [^1]
- It is the <mark style="background: #FF5582A6;">most important distribution</mark> in [[202602062059 - statistics|Statistics]] because of the [[202602080014 - central limit theorem|Central Limit Theorem]], which states that the averages of [[202602061340 - random variable|random variables]] independently drawn from any distribution converge to a normal distribution

> [!MATH] Gaussian Probability Density Function
> The probability density for a value $x$ is:
> $$f(x) = \frac{1}{\sigma\sqrt{2\pi}} e^{-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^2}$$
> 
> **Key Parameters:**
> 1.  **Mean ($\mu$)**: The center/peak of the bell
> 2.  **Standard Deviation ($\sigma$)**: The width of the bell (how "spread out" the data is)

- [[202602062110 - mean|Mean]] = [[202602062113 - median|Median]] = [[202602062115 - mode|Mode]]
- In a Normal Distribution, where $\sigma$ is [[202602062209 - standard deviation|Standard Deviation]]
	- ~68% of data falls within $1\sigma$ of the mean
	- ~95% of the data falls within $2\sigma$ of the mean
	- ~99.7% of the data falls within $3\sigma$ of the mean


[^1]:  [The Normal Distribution, Clearly Explained!!!](https://www.youtube.com/watch?v=rzFX5NWojp0&list=PLblh5JKOoLUK0FLuzwntyYI10UQFUhsY9&index=3)