...
> $$k_1=D_{\mathrm{KL}}(P\|Q)=\mathbb{E}_P [\log \frac{P(x)}{Q(x)}]$$
...
- Control Variate
	- To reduce variance, we add a term with an expectation of zero

> [!MATH] Control Variate
> 
> - Let $r = \frac{Q(x)}{P(x)}$
> - We know that $\mathbb{E}_P[r] = \mathbb{E}_P[\frac{Q(x)}{P(x)}] = \int Q(x) dx = 1$, Thus:
> $$\mathbb{E}_P[r - 1] = 0$$

- Any estimator $k = k_1 + \lambda(r - 1)$ remains unbiased for any $\lambda$
- The Schulman "k3" Estimator
	- Schulman chose $\lambda = 1$ because it ensures the estimator is **always non-negative**
	- This is derived from the property that $x - 1 \ge \log x$ for all $x > 0$

> [!MATH] The Schulman "k3" Estimator
> $$k_3 = (r - 1) - \log(r)$$
> - Substituting $r = \frac{Q(x)}{P(x)}$ and noting that $-\log(r) = \log(1/r) = \log \frac{P(x)}{Q(x)}$:
> $$k_3 = \frac{Q(x)}{P(x)} - \log \left( \frac{Q(x)}{P(x)} \right) - 1$$
> - $r$: The probability ratio $\frac{Q(x)}{P(x)}$
> - $k_3$: The Schulman estimator; it is unbiased, non-negative, and has lower variance than $k_1$ when $P$ and $Q$ are close
...
[^4]: [Fantastic KL Divergence and How to (Actually) Compute It](https://www.youtube.com/watch?v=tXE23653JrU)